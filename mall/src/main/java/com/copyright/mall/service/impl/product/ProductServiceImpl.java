...
public class ProductServiceImpl implements IProductService {
    ...
    public ProductByClassVO getProductByClass(ProductByClassparam productByClassparam) {
        ...
        shop.setMallId(1L);
        List<Shop> shops = shopService.selectByObjectList(shop);
        ClassTwoParam param = new ClassTwoParam();
        param.setClassOneId(productByClassparam.getFirstCategoryId());
        List<ClassResp> classResps = classificationService.getClassTwo(param);
        List<Long> ids = classResps.stream().map(ClassResp::getFirstCategoryId).collect(Collectors.toList());
        PageHelper.startPage(productByClassparam.getPageNum(), productByClassparam.getPageSize());
        List<ClassItemRelation> classItemRelations = classItemRelationService.selectByIdsList(ids);
